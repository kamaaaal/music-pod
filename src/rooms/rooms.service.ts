...
export class RoomsService{
    ...
    public async createRoom (body : CreateRoomDto, user : Omit<UserEntity,'password'>){
        const roomCreated = await this.roomsRepository.save({
            roomName : body.roomName,
            createdBy : user,
        })
        // console.log(roomCreated)
        return createResponse(201,"Room created successfully",{roomId : roomCreated.roomId, roomName : roomCreated.roomName});
    }
